...
//
// Rich Insley -
// Modified to allow [] operator to resize when index outside of current size
//
//===----------------------------------------------------------------------===//
///
/// \file
...
#ifndef FLAVOR_SMALLVECTOR_H
#define FLAVOR_SMALLVECTOR_H
...
public:
  ...
  void pop_back() {
   ...
   // jl - move this from SmallVectorTemplateCommon and rub it to grow
   T& operator[](size_t idx) {
     // jl - resize to accomodate up to idxof instead fail
     size_t s = SmallVectorTemplateCommon<T>::size();
     if(idx >= s) {
         for(int i = 0; i < idx + 1 - s; i++) {
             T el;
             const T *EltPtr = reserveForParamAndGetAddress(el);
             ::new ((void *)this->end()) T(*EltPtr);
         }
         SmallVectorTemplateCommon<T>::set_size(idx + 1);
     }
     return SmallVectorTemplateCommon<T>::begin()[idx];
   }
};
...
public:
  ...
  void pop_back() { this->set_size(this->size() - 1); }
   ...
   const T& operator[](size_t idx) const {
     // jl - resize instead of fail
     assert(idx < SmallVectorTemplateCommon<T>::size());
     return SmallVectorTemplateCommon<T>::begin()[idx];
   }
};
...
namespace std {
  ...
  template<typename T>
  inline void
  swap(flavor::SmallVectorImpl<T> &LHS, flavor::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, unsigned N>
  inline void
  swap(flavor::SmallVector<T, N> &LHS, flavor::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
  }
